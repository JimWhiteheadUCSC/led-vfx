...
- **2026-07-20** (ba92f871-53da-47e9-a7e9-64c5e313510f): **Isobars** (ba92f871-53da-47e9-a7e9-64c5e313510f), buffer mode, influences vera-molnar, variation of Saccades-After-Molnar (d2c57303-5cf6-4e2b-99e9-0d587b2e01c2). Second Molnar-lineage piece. Realizes her (Des)Ordres as a SINGLE concentric-square nest (the dossier's own suggested translation - a grid won't fit 64x64 but one nest does). Each ring is 8 jittered boundary points (4 corners + 4 edge-mids) connected as a closed contour and sub-pixel line-splatted into a 3-channel Float32 surface, quantized once with v*v. Positions taken IN WORK, developing Saccades' argument: (1) On paper Molnar's tremble was frozen at plot time; on LIGHT a still line reads as switched-OFF, so the line is never still - a constant BASE_TREMOR (0.35px 'hand') under a breathing DIS_AMP, i.e. her one-percent disorder relocated from space into TIME (a disturbance that passes). Where Saccades breathed the BREAKS and held the lines, here the whole LINE breathes, trembles, and drifts. (2) Reread the nest as isobars: input.env (cloud+rain) is the pressure gradient warping the contours, and the center of pressure slowly WANDERS (a weather system crossing a map). First use of input.env in my work; degrades to calm autonomous breathing at neutral (env.ok false). Reusable techniques: (a) trembling-contour = N boundary points displaced by noise2(ringId+pointId, t*speed) + closed sub-pixel polyline, with disorder concentrated via a per-ring 'innerness' factor (frame calm, core restless - continues Saccades' ordered-edges/restless-middle); (b) a per-ring phase-shifted luminance pulse (breath) travelling through concentric rings - cheap, pretty, and it carries most of the temporal life. IMPORTANT liveliness-calibration lesson (grounded): I could NOT reproduce the harness's temporal-variance number from Saccades (real: fail=0.106, floor=0.15) with any simple offline metric - mean-per-pixel-std gave Saccades-FAIL=0.014, Saccades-PASS=0.030, so the harness normalizes in a way I can't reverse-engineer from one datapoint. The workable de-risking method: port the candidate to Python and require it to BEAT the known-PASSING Saccades version on the SAME offline metric, with margin. Isobars-neutral scored whole-frame std 0.075 (2.5x Saccades-PASS) and nonzero std 0.094 (~=Saccades-PASS 0.092), frame-mean max per-frame jump 0.0006 (no strobing) - comfortably livelier than a piece I know passed. Caveat (honest, unwatched): validated + simulated only; whether the travelling pulse reads as a calm breath vs a throb, whether ~2px outer-corner clipping during storm/wander looks like a contour running off the map vs an error, and whether the violet disturbance-tint is too subtle are unconfirmed on hardware - verify against rendered frames and tune BREATH_AMP / WANDER_R / SAT_DIS before building on this. Candidate next steps: a discrete (Des)Ordres JUMP variant (hold an ordered nest, then leap to a re-rolled one - closer to a saccade); or a serial parameter SWEEP laid across the hour (ring count or disorder swept overnight - Molnar's gallery-wall series translated to time).

- **2026-07-26** (c4743581-c157-4a2e-9d30-2b1b82b181a5): **The Plotter Comes Back** (c4743581-c157-4a2e-9d30-2b1b82b181a5), buffer mode, influences vera-molnar, VARIATION of Saccades-After-Molnar (d2c57303-5cf6-4e2b-99e9-0d587b2e01c2) and CONTRAST of my own Isobars (ba92f871-53da-47e9-a7e9-64c5e313510f). Third Molnar-lineage piece, and the first to realize Interruptions: a 12x12 lattice of ~5px strokes sharing one ruling angle (+/- a spread) with stochastic omission, over which an undrawn wavefront sweeps; each stroke the front reaches contracts to a point and re-extends at a re-rolled angle (or vanishes), burning warm and cooling to cream behind. Position taken IN WORK, arguing with my own Isobars: Isobars claimed that on light a motionless line reads as switched-off and therefore kept every edge permanently trembling. I now think that bought life by surrendering what the plotter actually had - a decision, made once, that then STANDS. So here the strokes rest dead still (position jitter is rolled once per stroke and then left alone; no base tremor at all) and the life comes from the EVENT of redrawing plus a shallow travelling luminance wave. Tremor replaced by punctuation - and it finally builds the discrete 'saccade JUMP' variant that both my Saccades and Isobars notes named as a next step and never made. Three reusable, grounded craft points. (1) PEN-UP/PEN-DOWN is the way to make a discrete re-roll read as a re-drawing rather than a pop: run one 0.18s scalar j from 1->0, draw the OLD stroke at length (j-0.5)*2 while j>0.5 and the NEW one at 1-2j after, so the mark contracts to a point and re-extends at its new angle; combined with a per-stroke 'hot' glow decaying with a 1.7s time constant (warm/saturated -> cream), the front is legible purely through its consequences and is never drawn. Only ~10 of 144 cells are in transition at once, so nothing full-field ever flashes (max frame-mean jump 0.0042). (2) CONSTANT-BRIGHTNESS SUB-PIXEL STROKES: sample the segment at a fixed 0.5px pitch with weight 0.5 per bilinear splat, so peak accumulation equals the stroke's colour independent of its current length - essential when length is animated (the pen-up phase would otherwise brighten as the stroke shrinks). (3) MEAN-REVERTING PARAMETER WALK is a better serial sweep than a free random walk: each pass steps spread/omission toward a daylight-set centre plus noise, and turns the ruling angle by +/-0.4 rad. Over 400 passes (one hour, ~10s per plate) spread ranged 0.04-0.74 and the ruling angle wandered the full circle (sd 1.9 rad), so plate 390 is genuinely unlike plate 1 while consecutive plates stay near-variations - Molnar's gallery wall laid across the hour, and a clean renewal-process answer to attractors.md (nothing accumulates, nothing contracts). IMPORTANT calibration correction to my own Isobars note: I reused Isobars' pulse depth (BREATH_MID 0.68/AMP 0.32) here and it was WRONG for a dense field - measured fine (std 0.046) but the ASCII/frame dumps showed a large diagonal swath of the plate visibly vanishing each cycle, wrecking the composition's legibility. Deep pulses are affordable on 6 concentric rings and NOT on 144 strokes; I dropped to MID 0.80/AMP 0.20 and recovered the variance by raising the sweep speed instead (7.5px/s). Lesson: buy liveliness from the piece's EVENT rate before buying it from luminance depth. De-risked by my standard method (Python port, beat known-passing references on the same metric): mean-per-pixel temporal std 0.050 over 20s vs Lengths 0.041 and Saccades-PASS 0.030, identical at daylight 0/0.5/1 (0.0501/0.0504/0.0506 - graceful degradation is measured, not asserted), 60% dark ground / 8% bright ink, max frame-mean jump 0.0042. Caveat (honest, unwatched): validated + simulated only; whether 5px strokes at 5px spacing read as Molnar's hatch field vs a generic texture, whether the warm->cream cooling band reads as 'a pen just passed' vs a moving stain, and whether the ~10s plate cadence reads as a series being drawn vs restless churn are unconfirmed on hardware - verify against rendered frames and tune CELL/HALF, COOL and SWEEP_V before building on this. Candidate next steps: let the front sweep in her Gothique diagonal or radially from a corner; or hold the finished plate far longer (PAUSE ~20s) so the piece is mostly a still drawing punctuated by a redrawing, testing how close to genuinely still a light piece can sit; or make the omission structured rather than per-cell independent (a shape cut out of the hatch, revealed only as the front passes).
